  ...
  try {
    ...
    books = res.results.map((book) => {
      return {
        ...
        author: book.properties["Author(s)"].multi_select[0]?.name
          ? book.properties["Author(s)"].multi_select[0].name
          : "",
        publisher: book.properties.Publisher.multi_select[0]?.name
          ? book.properties.Publisher.multi_select[0].name
          : "",
      };
    });
...
async function getBook(books) {
  ...
  const url =
    `https://www.googleapis.com/books/v1/volumes?q=${bookTitle}+inauthor:${bookAuthors}&key=${googleApiKey}`;
